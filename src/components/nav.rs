...
use crate::components::svg::{
    flags::{ Globe, America, Japan, Korea , Vietnam},
    themes::{ Light, Dark, Rust },
};

#[function_component(Nav)] pub fn nav() -> Html {
	...
	<header class="navheader">
		...
		<span class="logo__text">{ "Duc" }</span>
	    </a>
		...
		<ul class="nav__items">
			...
			style={ if *show_lang_dropdown { "display:block;" } else { "display:none;" } } >
			    ...
			    <li onclick={ change_language("de", app_context.language.clone()) }><Vietnam />{ "Việt" }</li>
